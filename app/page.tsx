...
function statusOf(item: SearchItem) {
  return (
    ...
    item.notes ||
    item.status ||
    "Sea-to-Land Engine data record"
  );
}
  ...
  const proofTrail = useMemo(() => {
    ...
    if (activeItem.category === "Route") {
      ...
      database.places.forEach((place) => {
        if (
          (activeItem.path || []).some(
            (point: string) =>
              place.name.includes(point) || point.includes(place.name)
          )
        ) {
          places.add(place);
        }
    ...
    if (activeItem.category === "Registry") {
      registries.add(activeItem);
    }

    if (activeItem.category === "Business") {
      businesses.add(activeItem);
    }
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-slate-950 text-white">
      ...
      <section className="mx-auto max-w-7xl px-6 py-12">
        <div className="rounded-3xl border border-cyan-400/30 bg-slate-900/90 p-8 shadow-2xl">
          ...
          <p className="mt-4 max-w-3xl text-slate-300">
            A Sea-to-Land historical research engine for memory, genealogy, land
            titles, migration routes, proof trails, public registries, and
            restoring the land back through truth, records, and living history.
          </p>

          <p className="mt-4 max-w-3xl text-lg font-semibold text-amber-200">
            From the King of Water to the memory of the land — restoring what
            was lost, proving what remains, and building the archive for future
            generations.
          </p>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <a
              href="/history"
              className="inline-block rounded-xl border border-amber-400/40 bg-amber-400/10 px-5 py-3 text-sm font-bold uppercase tracking-widest text-amber-200 hover:bg-amber-400/20"
            >
              Open Living History Document
            </a>

            <a
              href="#public-research-links"
              className="inline-block rounded-xl border border-cyan-400/40 bg-cyan-400/10 px-5 py-3 text-sm font-bold uppercase tracking-widest text-cyan-200 hover:bg-cyan-400/20"
            >
              Open Public Research Links
            </a>
          </div>
        </div>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-4">
          ...
          >
            ...
            <p className="mt-2 text-sm text-slate-300">
              People, places, records, parcels, routes, sources, and proof
              status already inside your Engine.
            </p>
          </button>
          ...
          >
            ...
            <p className="mt-2 text-sm text-slate-300">
              Land titles, government records, archives, newspapers,
              FamilySearch, WikiTree, and official research doors.
            </p>
          </button>
          ...
          >
            ...
            <p className="mt-2 text-sm text-slate-300">
              Connect memory, land parcels, migration routes, and sources into
              a living restoration archive.
            </p>
          </button>
          ...
          >
            ...
            <p className="mt-2 text-sm text-slate-300">
              Friend-submitted businesses, services, sales links, partners, and
              project supporters.
            </p>
          </button>
        </div>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-4">
          ...
          <Stat
            icon={<FileText />}
            label="Records"
            value={database.records.length}
          />
          <Stat
            icon={<Database />}
            label="Parcels"
            value={database.parcels.length}
          />
          <Stat icon={<Route />} label="Routes" value={database.routes.length} />
          <Stat
            icon={<Archive />}
            label="Sources"
            value={database.sources.length}
          />
          <Stat
            icon={<Archive />}
            label="Registries"
            value={database.registries.length}
          />
          <Stat
            icon={<Users />}
            label="Businesses"
            value={database.businesses.length}
          />
        </div>
        ...
        <section className="mt-8 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-6">
          ...
          <p className="mt-2 text-sm text-slate-300">
            These badges separate embedded facts, source targets, public leads,
            family tradition, and story layers so the Engine keeps truth,
            research, and restoration clearly marked.
          </p>
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_420px]">
          <section>
            <h2 className="text-2xl font-bold">
              {mode === "archive" && "My Archive Only"}
              {mode === "public" && "Public Research Waters"}
              {mode === "restore" && "Restore the Land"}
              {mode === "market" && "Community Market"}
            </h2>

            <p className="mt-2 text-sm text-slate-400">
              {mode === "archive" &&
                "Searching only your embedded facts: people, places, records, parcels, routes, and sources."}
              {mode === "public" &&
                "Searching outside research doors: registries, archives, land titles, newspapers, and government records."}
              {mode === "restore" &&
                "Showing land, route, source, and proof records connected to restoration."}
              {mode === "market" &&
                "Showing friend-submitted businesses, services, sales links, and project supporters."}
            </p>
              ...
              {allResults.map((item, index) => (
                ...
                >
                  ...
                  <p className="mt-2 text-slate-300">
                    {item.description || item.type || item.notes || statusOf(item)}
                  </p>
        ...
        <section
          id="public-research-links"
          className="mt-10 rounded-2xl border border-cyan-400/30 bg-slate-900 p-6"
        >
...
function ProofTrail({ item, trail }: { item: SearchItem | null; trail: any }) {
    ...
    return (
      <aside className="rounded-2xl border border-white/10 bg-slate-900 p-6">
        ...
        <p className="mt-3 text-slate-400">
          Select a result to see connected evidence.
        </p>
      </aside>
    );
  }
  ...
  return (
    <aside className="h-fit rounded-2xl border border-cyan-400/30 bg-slate-900 p-6">
      ...
      {database.registries.length > 0 && (
        <section className="mt-6 rounded-2xl border border-cyan-400/30 bg-slate-950 p-5">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            {database.registries.map((registry) => (
              ...
              >
                ...
                <h4 className="mt-2 text-sm font-bold">{registry.name}</h4>

                <p className="mt-2 text-xs text-slate-300">{registry.use}</p>
                ...
                {registry.url && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-3 inline-block rounded-lg border border-cyan-400/40 bg-cyan-400/10 px-3 py-2 text-xs font-bold text-cyan-200 hover:bg-cyan-400/20"
                  >
                    Open Registry
                  </a>
                )}
      ...
      <TrailList title="Linked Businesses" items={trail.businesses} />
    </aside>
  );
}
